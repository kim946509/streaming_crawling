...
def test_genie_crawling(songs):
    ...
    # SongInfo 객체를 Genie 크롤링 형식으로 변환 (song_id 포함)
    song_list = [
        {
            'song_id': song.id,
...
def test_youtube_music_crawling(songs):
    ...
    # SongInfo 객체를 YouTube Music 크롤링 형식으로 변환 (song_id 포함)
    song_list = [
        {
            'song_id': song.id,
            'song_title': song.song_name, 
...
def test_youtube_crawling(songs):
    ...
    # SongInfo 객체를 YouTube 크롤링 형식으로 변환 (song_id 포함)
    url_artist_song_id_list = [
        (song.youtube_url, song.artist_name, song.id)
        for song in songs_with_url
    ]
    
    try:
        results = run_youtube_crawling(url_artist_song_id_list, save_csv=True, save_db=True)
        ...
        print(f"[YouTube] 곡당 평균 시간: {format_time(elapsed_time / len(url_artist_song_id_list)) if url_artist_song_id_list else '0초'}")
        print(f"[YouTube] 성공한 곡 수: {len(results)}/{len(url_artist_song_id_list)}")
